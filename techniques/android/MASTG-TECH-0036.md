...
Note that performing emulation can leave indicators that apps detect at runtime (for example, `Build` values, telephony identifiers, package names, or graphics renderer strings). See @MASTG-KNOW-0031 for common emulator indicators that can be detected by applications.
