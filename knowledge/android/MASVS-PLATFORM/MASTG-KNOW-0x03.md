...
Broadcasts are built on top of the [`Intent`](https://developer.android.com/reference/android/content/Intent) system and are an inter-process communication (IPC) entry point. See @MASTG-KNOW-0020 for the IPC model and @MASTG-KNOW-0025 for implicit intents.
...
- [`android:exported`](https://developer.android.com/guide/topics/manifest/receiver-element#exported): when `true`, other apps can send broadcasts to the receiver. When a manifest receiver declares an intent filter, the default historically became `true`; in its absence the default is `false`. Since Android 12 (API level 31) the attribute must be set explicitly when an intent filter is present.
- [`android:permission`](https://developer.android.com/guide/topics/manifest/receiver-element#prmsn) on the receiver, or a receiver permission passed to `sendBroadcast(intent, receiverPermission)`, restricts which senders or receivers participate based on a held permission.
